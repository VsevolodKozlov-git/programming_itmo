...
def group(values: List[str], n: int) -> List[List[str]]:
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    return [values[row*n: (row+1) * n] for row in range(math.ceil(len(values) / n))]
...
def get_block(grid: List[List[str]], pos: Tuple[int, int]) -> List[str]:
    ...
    for row_ind in range(start_row, start_row+3):
        for col_ind in range(start_col, start_col+3):
           res.append(grid[row_ind][col_ind])
    return res
...
def solve(grid: List[List[str]]) -> Optional[List[List[str]]]:
    ...
    if empty_pos is None:
        return grid
    values = list(find_possible_values(grid, empty_pos))
    if len(values) == 0:
        return None
    while True:
        grid_copy = copy.deepcopy(grid)
        grid_copy[empty_pos[0]][empty_pos[1]] = values.pop()
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    def check_arr(arr: list):
        if len(set(arr)) != len(arr) or "." in arr:
            return False
        return True

    for row_ind in range(9):
        if not check_arr(get_row(solution, (row_ind, 0))):
            return False

    for col_ind in range(9):
        if not check_arr(get_row(solution, (0, col_ind))):
            return False

    blocks = [(row, col) for col in range(0, 9, 3) for row in range(0, 9, 3)]
    for block in blocks:
        if not check_arr(get_block(solution, block)):
            return False
